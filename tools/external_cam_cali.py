...
class vCamera:
    ...
    def open(self) -> List[int]:
        ...
        errors.append(mvsdk.CameraSetTriggerMode(self.h_camera, self.trigger))  # Set camera to continuous capture mode
        if True:  # conti or soft
            ...
            if not self.auto_exposure:
                errors.append(
                    mvsdk.CameraSetFrameSpeed(self.h_camera,
                                              self.frame_speed))  # When long exposure is required, frame_speed needs to be set to 0 (slow speed)
                errors.append(mvsdk.CameraSetExposureTime(self.h_camera, int(self.exposure_ms * 1000)))
            mvsdk.CameraSetWbMode(self.h_camera, 0)

        errors.append(mvsdk.CameraPlay(self.h_camera))  # Start the internal image capture thread in SDK
